...
> （192.168.123.24 / 192.168.60.162），`config.yaml` 指向上述地址时启动会被拒绝。
...
**响应/统计字段**：`total/sent/errors/rate/remainingS/elapsed/startTime/endTime/running`。

> 生成任务达到 `total` 后自动停止；运行中重复启动返回 HTTP 409。
> 指向生产平台（192.168.123.24 / 192.168.60.162）启动时返回 HTTP 400。
...
1. 将 config.yaml 的 sasoc.host 改为 `192.168.123.24`（或 `192.168.60.162`）
