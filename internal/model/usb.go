...
func InsertUsb(db *sql.DB, u *SimUsbRow) error {
	_, err := db.Exec(`INSERT INTO sim_usb
		(id, sn, model, firmware_version, qualified, area_name, claim_info, inserted, station_id, door_no)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func GetUsb(db *sql.DB, id string) (*SimUsbRow, error) {
	row := db.QueryRow(`SELECT id, sn, model, firmware_version, qualified, area_name, claim_info, inserted, station_id, door_no
		FROM sim_usb WHERE id = ?`, id)
	var u SimUsbRow
	if err := row.Scan(&u.ID, &u.SN, &u.Model, &u.FirmwareVersion, &u.Qualified, &u.AreaName,
		&u.ClaimInfo, &u.Inserted, &u.StationID, &u.DoorNo); err != nil {
...
func ListUsbs(db *sql.DB) ([]*SimUsbRow, error) {
	rows, err := db.Query(`SELECT id, sn, model, firmware_version, qualified, area_name, claim_info, inserted, station_id, door_no
